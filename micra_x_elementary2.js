(function(ext){
  ...
  function drawCircle(id, data, radius, x, y, z, pit, rot){
    $.ajax({
      type: "GET",
  ...
  ext.drawCircle = drawCircle;

  function drawEllipse(id, data, radius, ratio, x, y, z, st, en, pit, rot){
    $.ajax({
      type: "GET",
  ...
  ext.drawEllipse = drawEllipse;

  function drawArc(id, data, radius, x, y, z, st, en, pit, rot){
    $.ajax({
      type: "GET",
  ...
  function drawArcRadis(id, data, radius, x, y, z, st, en, pit, rot){
    $.ajax({
      type: "GET",
  ...
  ext.drawArcRadis = drawArcRadis;

  function drawBall(id, data, radius, x, y, z){
    $.ajax({
      type: "GET",
  ...
  ext.strokePen = strokePen;

  function turnPen(rot, pit){
    $.ajax({
      type: "GET",
  ...
  var CARPETs = [
   ...
   function carpet_NameToID(bName) {
    ...
    ext.pre_drawCircle = pre_drawCircle;
    ...
    ext.pre_drawEllipse = pre_drawEllipse;
    ...
    ext.pre_drawArcRadis = pre_drawArcRadis;

    function pre_drawBall(IdData, x, y, z, radius){
        drawBall(IdData[0], IdData[1], radius, x, y, z);
    }
    ext.pre_drawBall = pre_drawBall;

    function pre_drawCube(IdData, x, y, z, size){
        setBlocks(IdData[0], IdData[1], x, y, z, x+size, y+size, z+size);
    }
    ext.pre_drawCube = pre_drawCube;

    function pre_setPen(x, y, z, rot){
        setPen(x, y, z, rot, 0);
    }
    ext.pre_setPen = pre_setPen;

    function pre_downPen(IdData){
        downPen(IdData[0], IdData[1]);
    }
    ext.pre_downPen = pre_downPen;

    function pre_turnPen(rot, pit){
        turnPen(rot, 0);
    }
    ext.pre_turnPen = pre_turnPen;
  ...
  var descriptor = {
    blocks: [
        ...
        [" ", "%s の文字列 %s 背景を%s フォントを%m.Font x:%n y:%n z:%n", "pre_drawText"," ","Great!"," ","8x8",0,0,0],
        [" ", "テレポートする x:%n y:%n z:%n", "Teleport",0,0,0],
        [" ", "周囲をリセット", "ResetHere"],
        [" ", "原点でリセット", "Reset"],
        [" ", "%s の弧 x:%n y:%n z:%n 半径:%n で%m.Rot ° 開いて%m.Pit", "pre_drawArc"," ",0,0,0,10,"180","立てる"],
        [" ", "%s の円 x:%n y:%n z:%n 半径:%n で%m.Pit", "pre_drawCircle"," ",0,0,0,10,"立てる"],
        [" ", "%s のおうぎ x:%n y:%n z:%n 半径:%n で%m.Rot ° 開いて%m.Pit", "pre_drawArcRadis"," ",0,0,0,10,"180","立てる"],
        [" ", "%s のだ円 x:%n y:%n z:%n 半径:%n で%m.Pit", "pre_drawEllipse"," ",0,0,0,10,"立てる"],
        [" ", "%s の立方体 x:%n y:%n z:%n 大きさ:%n", "pre_drawCube"," ",0,0,0,10],
        [" ", "%s の球 x:%n y:%n z:%n 半径:%n", "pre_drawBall"," ",0,0,0,10],
        [" ", "ペンの位置を x:%n y:%n z:%n 向きを %m.Rot ° に設定", "pre_setPen",0,0,0,"0"],
        [" ", "%s のペンを下ろす", "pre_downPen"," "],
        [" ", "ペンを長さ %n ブロック分進める", "pre_strokePen",1],
        [" ", "ペンの向きを 右に%m.Rot ° ずつ変える", "pre_turnPen","30"],
        [" ", "ペンを上げる", "upPen"],
    ...
    menus: {
        Pit: ['立てる','ねかす'],
        Rot: ['0','30','45','60','90','120','135','150','180','210','225','240','270','300','315','330','360'],
        ...
        reds: ['ディスペンサー','音符ブロック','パワードレース','ディテクターレール','ピストン','TNT','モンスタースポナー','チェスト','レール','レバー','木の感圧版','石のボタン','コマンドブロック','木のボタン','日照センサー','ボタン'],
